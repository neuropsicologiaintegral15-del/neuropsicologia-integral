import { useState, useEffect } from "react";
import { motion, AnimatePresence } from "framer-motion";
import {
  FaTimes,
  FaGraduationCap,
  FaBriefcase,
  FaStar,
  FaCrown,
} from "react-icons/fa";
import { IoIosPaper } from "react-icons/io";
import Footer from "../../components/footer/Footer";
import TestimonialsCarousel from "../../components/testimonias-carousel/TestimonialsCarousel";
import BackButton from "../../components/back-button/BackButton";
// Importar desde el archivo de datos
import { teamMembers, type ProfessionalMember } from "./data/teamMembers";

export default function Professionals() {
  const [selectedMember, setSelectedMember] =
    useState<ProfessionalMember | null>(null);

  useEffect(() => {
    if (selectedMember) {
      document.body.style.overflow = "hidden";
    }

    return () => {
      document.body.style.overflow = "auto";
    };
  }, [selectedMember]);

  const openModal = (member: ProfessionalMember) => {
    setSelectedMember(member);
  };

  const closeModal = () => {
    setSelectedMember(null);
  };

  const featuredMember = teamMembers.find((member) => member.featured);
  // Ahora habrán 7 otros miembros (los 4 originales + los 3 nuevos)
  const otherMembers = teamMembers.filter((member) => !member.featured);

  return (
    <div className="min-h-screen bg-gray-50">
      {/* Botón de regreso */}
      <div className="container sticky top-0 mx-auto z-50">
        <BackButton
          className="inline-flex items-center text-white bg-primary-dark font-medium m-4 px-4 py-2 rounded-lg cursor-pointer shadow-xl"
          title="Volver"
          section="equipo"
        />
      </div>

      {/* Contenido principal */}
      <main className="container mx-auto px-4">
        {/* Introducción */}
        <div className="text-center mb-12">
          <div className="w-24 h-1 bg-primary-200 mx-auto mb-6"></div>
          <p className="text-gray-600 max-w-3xl mx-auto text-lg">
            "Somos un centro especializado en neuropsicología y salud mental,
            con un enfoque interdisciplinario, basado en evidencia científica y
            orientado al bienestar integral de niños, adolescentes y adultos."
          </p>
        </div>

        {/* VIVIANA DESTACADA - DISEÑO CENTRADO */}
        {featuredMember && (
          <div className="mb-16">
            <div className="text-center mb-8">
              <h3 className="text-2xl font-bold text-gray-800 mb-2 flex items-center justify-center">
                <FaCrown className="mr-3 text-yellow-500" />
                Directora y Fundadora
                <FaCrown className="ml-3 text-yellow-500" />
              </h3>
              <p className="text-gray-600">
                Liderazgo y experiencia al servicio de tu bienestar
              </p>
            </div>

            <motion.div
              className="bg-gradient-to-r from-blue-50 to-primary-50 rounded-2xl shadow-xl overflow-hidden"
              initial={{ opacity: 0, y: 30 }}
              animate={{ opacity: 1, y: 0 }}
              transition={{ duration: 0.6 }}
            >
              <div className="p-8">
                {/* IMAGEN CENTRADA */}
                <div className="flex flex-col items-center mb-8">
                  <div className="relative">
                    <img
                      src={featuredMember.image}
                      alt={featuredMember.name}
                      className="w-72 h-72 rounded-full object-cover border-8 border-white shadow-2xl"
                    />
                    <div className="absolute -top-2 -right-2 bg-yellow-500 text-white rounded-full p-3">
                      <FaCrown size={24} />
                    </div>
                  </div>

                  {/* INFORMACIÓN CENTRADA DEBAJO DE LA FOTO */}
                  <div className="mt-8 text-center max-w-2xl">
                    <h3 className="text-2xl font-bold text-gray-800 mb-2">
                      {featuredMember.name}
                    </h3>
                    <p className="text-primary-200 font-bold text-xl mb-2">
                      {featuredMember.title}
                    </p>
                    <p className="text-gray-700 mb-4">
                      {featuredMember.specialty}
                    </p>
                    <p className="text-gray-600 text-lg mb-6">
                      {featuredMember.shortDescription}
                    </p>
                  </div>
                </div>

                {/* BOTONES CENTRADOS */}
                <div className="flex flex-wrap justify-center gap-4 mb-8">
                  <button
                    onClick={() => openModal(featuredMember)}
                    className="bg-primary-200 hover:bg-primary-300 text-black font-medium py-3 px-8 rounded-lg transition-colors flex items-center text-lg shadow-lg hover:shadow-xl cursor-pointer"
                  >
                    <IoIosPaper className="mr-3" />
                    Ver Perfil Completo
                  </button>
                </div>
              </div>
            </motion.div>
          </div>
        )}

        {/* Título para el resto del equipo - ACTUALIZADO */}
        <div className="text-center mb-10">
          <h3 className="text-2xl font-bold text-gray-800 mb-2">
            Nuestro Equipo Especializado
          </h3>
          <p className="text-gray-600">
            {otherMembers.length} profesionales complementarios trabajando en
            equipo
          </p>
        </div>

        {/* Grid de los otros 7 profesionales - ACTUALIZADO A 3 COLUMNAS */}
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {otherMembers.map((member) => (
            <motion.div
              key={member.id}
              className="bg-white rounded-xl shadow-lg overflow-hidden hover:shadow-xl transition-shadow duration-300 h-full"
              initial={{ opacity: 0, y: 30 }}
              animate={{ opacity: 1, y: 0 }}
              transition={{ duration: 0.5 }}
            >
              <div className="p-6 h-full flex flex-col">
                <div className="flex flex-col items-center text-center mb-4">
                  {/* Imagen */}
                  <div className="mb-4">
                    <img
                      src={member.image}
                      alt={member.name}
                      className="w-32 h-32 rounded-full object-cover border-4 border-white shadow-lg mx-auto"
                    />
                  </div>

                  {/* Información básica */}
                  <div className="flex-grow">
                    <h3 className="text-xl font-bold text-gray-800 mb-2">
                      {member.name}
                    </h3>
                    <p className="text-primary-200 font-medium mb-2">
                      {member.title}
                    </p>
                    <p className="text-gray-600 text-sm mb-3">
                      {member.specialty}
                    </p>
                    <p className="text-gray-700 line-clamp-3 text-sm">
                      {member.shortDescription}
                    </p>
                  </div>
                </div>

                {/* BOTÓN CLARO Y GRANDE PARA VER PERFIL - CENTRADO */}
                <div className="mt-auto">
                  <button
                    onClick={(e) => {
                      e.stopPropagation();
                      openModal(member);
                    }}
                    className="w-full inline-flex items-center justify-center bg-primary-dark hover:bg-white text-white hover:text-black border-2 hover:border-black font-semibold py-3 px-4 rounded-lg transition-all duration-300 cursor-pointer"
                  >
                    <IoIosPaper className="mr-2" />
                    Ver Perfil Completo
                  </button>
                </div>
              </div>
            </motion.div>
          ))}
        </div>

        {/* Información del equipo - ACTUALIZADO */}
        <div className="mt-16 bg-white rounded-xl shadow-lg p-8">
          <h3 className="text-2xl font-bold text-gray-800 mb-6 text-center">
            Nuestro Equipo Multidisciplinario
          </h3>
          <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
            <div className="text-center p-4">
              <div className="text-primary-200 text-4xl font-bold mb-2">
                {teamMembers.length}
              </div>
              <div className="text-gray-700">Profesionales especializados</div>
            </div>
            <div className="text-center p-4">
              <div className="text-primary-200 text-4xl font-bold mb-2">5</div>
              <div className="text-gray-700">Áreas de especialidad</div>
            </div>
            <div className="text-center p-4">
              <div className="text-primary-200 text-4xl font-bold mb-2">
                80+
              </div>
              <div className="text-gray-700">Años experiencia colectiva</div>
            </div>
            <div className="text-center p-4">
              <div className="text-primary-200 text-4xl font-bold mb-2">
                100%
              </div>
              <div className="text-gray-700">Trabajo en equipo coordinado</div>
            </div>
          </div>
          <p className="text-gray-600 text-center mt-6 max-w-3xl mx-auto">
            Cada profesional aporta su experiencia específica, trabajando de
            manera coordinada bajo la dirección de Viviana Solís para ofrecerte
            una atención integral y personalizada. Nuestro equipo
            multidisciplinario cubre todas las etapas del ciclo vital, desde la
            primera infancia hasta la adultez.
          </p>
        </div>

        <TestimonialsCarousel />
      </main>

      {/* Modal para detalles completos - SIN CAMBIOS */}
      <AnimatePresence>
        {selectedMember && (
          <motion.div
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
            className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/70"
            onClick={closeModal}
          >
            <motion.div
              initial={{ opacity: 0, scale: 0.9, y: 50 }}
              animate={{ opacity: 1, scale: 1, y: 0 }}
              exit={{ opacity: 0, scale: 0.9, y: 50 }}
              className="bg-white rounded-2xl shadow-2xl max-w-4xl w-full max-h-[90vh] overflow-hidden"
              onClick={(e) => e.stopPropagation()}
            >
              {/* Header del modal */}
              <div className="flex justify-between items-center p-6 border-b border-gray-200">
                <div className="flex items-center">
                  {selectedMember.featured && (
                    <FaCrown className="text-yellow-500 mr-3" />
                  )}
                  <div>
                    <h2 className="text-2xl font-bold text-gray-800">
                      {selectedMember.name}
                    </h2>
                    <p className="text-primary-200 font-medium">
                      {selectedMember.title}
                    </p>
                    <p className="text-gray-600 text-sm">
                      {selectedMember.specialty}
                    </p>
                  </div>
                </div>
                <button
                  onClick={closeModal}
                  className="text-gray-500 hover:text-gray-700 p-2 rounded-full hover:bg-gray-100 transition-colors"
                >
                  <FaTimes size={24} />
                </button>
              </div>

              {/* Contenido del modal */}
              <div className="p-6 overflow-y-auto max-h-[calc(90vh-140px)]">
                <div className="flex flex-col lg:flex-row gap-8">
                  {/* Columna izquierda */}
                  <div className="lg:w-1/3">
                    <img
                      src={selectedMember.image}
                      alt={selectedMember.name}
                      className="w-full rounded-xl shadow-lg mb-6"
                    />
                    <div className="space-y-4">
                      <div>
                        <h3 className="font-bold text-gray-800 mb-2">
                          Descripción Profesional
                        </h3>
                        <p className="text-gray-700">
                          {selectedMember.shortDescription}
                        </p>
                      </div>
                    </div>
                  </div>

                  {/* Columna derecha */}
                  <div className="lg:w-2/3 space-y-8">
                    <div>
                      <h3 className="text-xl font-bold text-gray-800 mb-4">
                        Perfil Profesional Completo
                      </h3>
                      <div className="text-gray-700 whitespace-pre-line">
                        {selectedMember.fullDescription}
                      </div>
                    </div>

                    <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                      <div>
                        <h4 className="font-bold text-gray-800 mb-4 flex items-center">
                          <FaGraduationCap className="mr-2 text-primary-200" />
                          Formación Académica
                        </h4>
                        <ul className="space-y-3">
                          {selectedMember.education.map((item, index) => (
                            <li key={index} className="flex items-start">
                              <div className="w-2 h-2 bg-primary-200 rounded-full mt-2 mr-3"></div>
                              <span className="text-gray-600">{item}</span>
                            </li>
                          ))}
                        </ul>
                      </div>

                      <div>
                        <h4 className="font-bold text-gray-800 mb-4 flex items-center">
                          <FaBriefcase className="mr-2 text-primary-200" />
                          Experiencia
                        </h4>
                        <ul className="space-y-3">
                          {selectedMember.experience.map((item, index) => (
                            <li key={index} className="flex items-start">
                              <div className="w-2 h-2 bg-primary-200 rounded-full mt-2 mr-3"></div>
                              <span className="text-gray-600">{item}</span>
                            </li>
                          ))}
                        </ul>
                      </div>
                    </div>

                    <div>
                      <h4 className="font-bold text-gray-800 mb-4 flex items-center">
                        <FaStar className="mr-2 text-primary-200" />
                        Especialidades
                      </h4>
                      <ul className="space-y-3">
                        {selectedMember.specialties.map((specialty, index) => (
                          <li key={index} className="flex items-start">
                            <div className="w-2 h-2 bg-primary-200 rounded-full mt-2 mr-3"></div>
                            <span className="text-gray-600">{specialty}</span>
                          </li>
                        ))}
                      </ul>
                    </div>
                  </div>
                </div>
              </div>
            </motion.div>
          </motion.div>
        )}
      </AnimatePresence>

      <Footer />
    </div>
  );
}
